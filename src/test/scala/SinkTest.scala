...
class SinkTest extends FeatureSpec with Matchers {
  feature("Copy shopping list files locally") {
    scenario("copy one existing file") {
      ...
      val copiedFiles = Sink.copyToLocal("foo", Seq("https://upload.wikimedia.org/wikipedia/en/a/a9/Example.jpg").par, downloadFolder, HTTPCybernetic)
    ...
    scenario("copy non existing file") {
      ...
      an[FileNotFoundException] should be thrownBy Sink.copyToLocal("foo", Seq("http://foo", "https://upload.wikimedia.org/wikipedia/en/a/a9/Example.jpg").par, downloadFolder, nonExistingFileCybernetics)
    }
    ...
    scenario("copy not cloud allowed file") {
      ...
      an[IllegalArgumentException] should be thrownBy Sink.copyToLocal("foo", Seq("http://foo", "https://upload.wikimedia.org/wikipedia/en/a/a9/Example.jpg").par, downloadFolder, nonExistingFileCybernetics)
    }
